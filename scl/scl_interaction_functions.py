...
def production(particles, x, y, probability):
    p = particles[x][y]
    # 対象の近傍粒子２つをランダムに選ぶ
    n0_x, n0_y, n1_x, n1_y = get_random_2_moore_neighborhood(x, y, len(particles))
...
def disintegration(particles, x, y, probability):
    ...
    # 対象の近傍粒子をランダムに選ぶ 
    n_x, n_y = get_random_moore_neighborhood(x, y, len(particles))
...
def bonding(particles, x, y,
    ...
    if len(p['bonds']) >= 2 or len(n_p['bonds']) >= 2:
        return
    an0_x, an0_y, an1_x, an1_y = get_adjacent_moore_neighborhood(x, y, n_x, n_y, len(particles))
    if (an0_x, an0_y) in p['bonds'] or (an1_x, an1_y) in p['bonds']:
        return
    an0_x, an0_y, an1_x, an1_y = get_adjacent_moore_neighborhood(n_x, n_y, x, y, len(particles))
    ...
    mn_list = get_moore_neighborhood(x, y, len(particles)) + get_moore_neighborhood(n_x, n_y, len(particles))
